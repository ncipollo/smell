...
/// Same rules as [`file_complexity`], but parsed with the TSX grammar, which
/// `.ts` cannot use since it parses generics like `<T>(x)` as JSX instead.
pub fn tsx_file_complexity(source: &str, filter: &BranchFilter) -> FileComplexity {
    collector::file_complexity(
...
impl LanguageRules for TypeScriptRules {
    fn visit<'a>(&self, node: Node<'a>, source: &str) -> Visit<'a> {
        match node.kind() {
            "class_declaration" | "abstract_class_declaration" | "class" => Visit::Type(TypeDecl {
                name: class_name(node, source),
                supertypes: class_supertypes(node, source),
            }),
            "interface_declaration" => Visit::Type(TypeDecl {
                name: collector::field_text(node, "name", source),
                supertypes: extends_type_clause_supertypes(node, source),
            }),
...
/// Superclass and interfaces from a class's `class_heritage`: the `extends`
/// clause's expression (type arguments dropped; `--implements` strips
/// generics from both sides anyway) plus every type named in `implements`.
fn class_supertypes(node: Node, source: &str) -> Vec<String> {
    let Some(heritage) = collector::find_child(node, "class_heritage") else {
        return Vec::new();
    };
    let mut supertypes = Vec::new();
    if let Some(extends) = collector::find_child(heritage, "extends_clause") {
        supertypes.extend(field_values(extends, "value", source));
    }
    if let Some(implements) = collector::find_child(heritage, "implements_clause") {
...
/// An interface's supertypes come from its `extends_type_clause`.
fn extends_type_clause_supertypes(node: Node, source: &str) -> Vec<String> {
    let Some(clause) = collector::find_child(node, "extends_type_clause") else {
        return Vec::new();
    };
    field_values(clause, "type", source)
}

fn field_values(node: Node, field: &str, source: &str) -> Vec<String> {
    let mut cursor = node.walk();
...
mod tests {
    ...
    fn file_complexity_reports_supertypes() {
        let complexity = file_complexity(
            &testing::fixture("typescript/inherits.ts"),
    ...
    fn file_complexity_counts_only_selected_kinds() {
        ...
        let complexity = file_complexity(&testing::fixture("typescript/complexity.ts"), &filter);
        assert_eq!(
            testing::top_level_summary(&complexity),
            vec![
                ("simple".to_string(), 1),
    ...
    #[test]
    fn tsx_file_complexity_counts_branches_in_jsx() {
