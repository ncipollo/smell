...
pub fn check(reports: &[FileReport], options: &AnalysisOptions) -> Vec<CheckResult> {
    let configured = [
        ...
        (Measure::Comments, options.max_comments),
    ];
    configured
        .into_iter()
...
fn result(reports: &[FileReport], measure: Measure, limit: usize) -> CheckResult {
    let failures = match measure {
        ...
        Measure::Comments => comment_count::failures(reports, limit),
    };
    CheckResult {
        measure,
        limit,
...
mod tests {
    ...
    fn measure_names_are_stable_identifiers() {
        ...
        assert_eq!(Measure::Comments.name(), "comments");
    }
    ...
    fn all_configured_limits_run_in_declaration_order() {
        ...
        assert_eq!(results[5].measure, Measure::Comments);
    }
